...
define([
  ...
  function (
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin, Evented], {
      ...
      postCreate: function () {
        this.parcelPolygonGraphicsLayer = null;
        this.parcelLinesGraphicsLayer = null;
        this.parcelPointsGraphicsLayer = null
        this.rotationPointsGraphicsLayer = null;
      ...
      /**
      * Remove all graphics layer once widget is destroy
      * @memberOf widgets/ParcelDrafter/NewTraverse
      **/
      destroy: function () {
        if (this.parcelPolygonGraphicsLayer) {
          this.map.removeLayer(this.parcelPolygonGraphicsLayer);
        }
        if (this.parcelLinesGraphicsLayer) {
          this.map.removeLayer(this.parcelLinesGraphicsLayer);
        }
        if (this.parcelPointsGraphicsLayer) {
          this.map.removeLayer(this.parcelPointsGraphicsLayer);
        }
        if (this.rotationPointsGraphicsLayer) {
          this.map.removeLayer(this.rotationPointsGraphicsLayer);
        }
      },
        ...
        geometryUtils.getProjectedGeometry(mapPoint, defaultStartPointSpatialRef,
          this.geometryService).then(
            lang.hitch(this, function (projectedGeometry) {
              ...
              //if the user is clicking the same parcel point twice and distance is 0
              //then set the bearing textbox empty.
              if (distance === 0) {
                quadrantAngle = "";
              }
      ...
      **/
      _getMiscloseDetailsAccordingToPlanSettings: function (miscloseDetails) {
        ...
        miscloseDistance = this._getRoundedValue(miscloseDetails.LengthConversions,
          "MiscloseDistance");
        returnVal.miscloseDistance = jimuUtils.localizeNumber(Number(miscloseDistance)) + " " +
        ...
        calculatedArea = jimuUtils.localizeNumber(Number(calculatedArea)) + " " +
